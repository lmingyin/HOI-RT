            ...
            for id in positive_index:
                ...
                if cls == cls_sel:
                    ...
                    if len(role_bbox) > 1:
                        ...
                        if not np.isnan(bb_object[2]):
                            out_file.write(str(id_index) + " " + " ".join([str(a) for a in bb_agent]) + " " + " ".join([str(a) for a in bb_object]) + '\n')
                            out_file.close()
                       # else:
                       #     os.remove('%s/labels/%s/test/%s.txt' % (dataDir, cls, file_name[:-4]))
